...
if(@$_POST['formName']=='filter-sort'){
    ...
    $finishes=$_POST['finishes']=='' ? '' : ' OR prod_finishes IN ('.$_POST['finishes'].') ';
    $stoneCats=$_POST['stoneCats']=='' ? '' : ' OR prod_stone_cat IN ('.$_POST['stoneCats'].') ';
    $colors=$_POST['colors']=='' ? '' : ' OR prod_primary_color IN ("'.$_POST['colors'].'") ';
    $sizes=$_POST['sizes']=='' ? '' : ' OR ( prod_slab_size1 IN ("'.$_POST['sizes'].'") OR prod_slab_size2 IN ("'.$_POST['sizes'].'") )';
    $sortVal=$_POST['sortVal']==0 ? "created_date DESC" : ( $_POST['sortVal']==1 ? 'prod_name ASC' : 'prod_cat DESC' ) ;
    $selectProds=$dbConn->execute("SELECT prod_id,prod_stone_cat,prod_name,prod_primary_color , prod_slab_size1 , prod_slab_size2 ,prod_stone_cat , prod_finishes ,prod_images From products_data WHERE prod_images IS NOT NULL AND prod_status!=1 $finishes $stoneCats $colors $sizes ORDER BY $sortVal limit $loadMore");
    $products='';
